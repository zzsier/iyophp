...
<div id="xzw_wp">
  <div id="toptb" class="cl"> 
    ...
    <div class="wp">
      ...
      <div class="y"> 
        <!--{subtemplate common/xzw_top_search}--> 
        ...
        <!--{else}--> 
        <!--{hook/global_usernav_extra1}--> 
        <a href="member.php?mod=logging&action=logout&formhash={FORMHASH}" class="xzw_inline">{lang logout}</a> <strong class="vwmy qq">{$_G[member][username]}</strong> {lang usergroup}: $_G[group][grouptitle] 
        <!--{/if}--> 
...
<div class="ic_header">
  <div class="wp">
    ...
    <div class="mod_logo">
            ...
            <h2><!--{if !isset($_G['setting']['navlogos'][$mnid])}--><a href="{if $_G['setting']['domain']['app']['default']}http://{$_G['setting']['domain']['app']['default']}/{else}./{/if}" title="$_G['setting']['bbname']">{$_G['style']['boardlogo']}</a><!--{else}-->$_G['setting']['navlogos'][$mnid]<!--{/if}-->
            </h2>      
        </div>
        <!--{subtemplate common/xzw_top_search_simple}--> 
    </div>
  </div>
</div>
