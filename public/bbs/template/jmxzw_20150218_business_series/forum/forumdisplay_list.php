				...
				<!--{if CURMODULE != 'guide'}-->
					<div class="tf">
						...
						<!--{if $_GET['filter'] == 'hot'}-->
							...
							<span name="hottime" id="hottime" value="$ctime" onpropertychange="viewhot(this);" oninput="if(event.keyCode==13) {viewhot(this); doane(event);}" fid="$_G[fid]" onclick="showcalendar(event, this, false, false, false, false, function(){viewhot(this);});">$ctime</span>
						<!--{/if}-->
...
<div class="bm bw0 pgs cl">
  ...
  <!--{if !$_GET['archiveid']}--><a href="javascript:;" id="newspecialtmp" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}"  class="xzw_newspecial"></a><!--{/if}--> 
