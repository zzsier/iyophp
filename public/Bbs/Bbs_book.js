...
(function($, window, document, undefined) {
    ...
    $.fn.lazyload = function(options) {
        ...
        var settings = {
            ...
            appear: null,
            load: null,
            placeholder: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAMAAAAoyzS7AAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAAZQTFRF/Pz8AAAAhkN07gAAAAxJREFUeNpiYAAIMAAAAgABT21Z4QAAAABJRU5ErkJggg=="
        };
        function update() {
            var counter = 0;
                    ...
                    if (!this.loaded) {
                        ...
                        $("<img />").bind("load",
                            function() {
                                ...
                                var boxWidth = $("#bookContent").width();
                                var imgWidth = $self.attr("width");
                                var imgHeight = parseInt($self.attr("height"));
                                if (imgWidth != $self.attr("data-width")) {
                                    imgWidth = $self.attr("data-width")
                                }
                                if (boxWidth < imgWidth) {
                                    $self.attr("width", boxWidth).attr("height", parseInt(boxWidth * (imgHeight / imgWidth)))
                                } else {
                                    $self.attr("width", imgWidth)
                                }
                                imgWidth = $self.attr("width");
                                imgHeight = $self.attr("height");
                                if (imgWidth >= 200) {
                                    var picid = $self.attr("data-picid");
                                    var _width = imgWidth;
                                    var _left = $self.offset().left;
                                    var _top = parseInt($self.offset().top) + parseInt(imgHeight) - 36;
                                    $("#picReplyLayer_" + picid).fadeOut().css({
                                        width: _width,
                                        top: _top,
                                        left: _left
                                    }).fadeIn()
...
var btmLogin = {
    ...
    getPswStr: '<a href="http://my.zol.com.cn/index.php?c=getPassword">\u5fd8\u8bb0\u5bc6\u7801?</a>',
    createHtml: function() {
        var self = this,
...
ZeroClipboard.Client.prototype = {
    ...
    customReposAll: function(all) {
        $.each(all,
            function(key, val) {
                var top = $(val.div).prev().position().top;
                $(val.div).css({
                    "top": top
                })
            })
...
Book.replyMinLen = 5;
Book.isCurReply = 0;
Book.replyTips = '<p><a href="http://jindou.zol.com/" target="_blank">Z\u91d1\u8c46\u5151\u6362\u793c\u54c1&nbsp;&gt;&gt;</a></p>';
Book.replyBtm = $(".loadding-more").length ? $(".loadding-more") : $(".pgs-foot");
Book.toTop = function() {
    var userid = $.cookie("zol_userid");
    var data = {
        userid: userid,
    ...
    $.post("/index.php?c=Ajax_Book&a=toTop", data,
        function(json) {
            if (typeof json != "undefined" || !json) {
                if (json.info == "err") {
                    Layer.tips({
...
Book.toLight = function() {
    ...
    $.post("/index.php?c=Ajax_Book&a=toLight", data,
        function(json) {
            if (typeof json != "undefined" || !json) {
                if (json.info == "err") {
                    Layer.tips({
...
Book.getFriends = function() {
    if ($("#G_inviteFriend").length > 0) {
        $("#G_inviteFriend").remove()
    }
    Layer.inviteFriends();
    var userid = $.cookie("zol_userid");
    var data = {
    ...
    $.post("/index.php?c=Ajax_Book&a=getFriends&t=" + (new Date()).getTime(), data,
        function(json) {
            if (typeof json != "undefined" || !json) {
                if (json.info == "ok") {
                    var _li = "";
                    $.each(json.data,
                        function(key, val) {
                            _li += '<li><label class="label-check"><input name="friend" type="checkbox" value="' + key + '">' + val + "</label></li>"
                        });
                    $("#G_inviteFriend #G_inFriendList").append(_li);
                    App.toCenter("#G_inviteFriend");
                    $("#G_inviteFriend").show()
                } else {
                    if (typeof json.flag !== "undefined" && json.flag == "no") {
                        $.get("/index.php?c=Ajax_Book&a=recommendFollow&t=" + (new Date()).getTime(), {},
                            function(json) {
                                $("G_inviteFriend").remove();
                                $("#" + json.id).remove();
                                $("body").append(json.html)
...
Book.inviteFriends = function() {
    var data = {
        bbsid: $("#G_inviteForm").find("input[name=bbsid]").val(),
        boardid: $("#G_inviteForm").find("input[name=boardid]").val(),
        bookid: $("#G_inviteForm").find("input[name=bookid]").val(),
        friends: "",
        PScontent: $("#G_inviteForm .add-text-area").val()
    };
    $("#G_inviteForm").find("input[name=friend]").each(function() {
        if ($(this).is(":checked")) {
            data.friends += $(this).val() + "#"
        }
    });
    $.post("/index.php?c=Ajax_Book&a=inviteFriends", data,
        function(json) {
            if (typeof json != "undefined" || !json) {
                if (json.info == "err") {
                    Layer.tips({
                ...
                if (json.info == "ok") {
                    ...
                    typeof taskGuide !== "undefined" && taskGuide.jumpTo(location.href.toString(), 1500)
                }
            }
        },
        "json")
};
Book.reply = function(content, type, length) {
    if (typeof Book.isCurReply != "undefined" && Book.isCurReply == 1) {
        return false
    }
    ...
    var callback = function() {};
    if (type == "bottom") {
        var is_to_author = $("#toAuthor").val() ? parseInt($("#toAuthor").val()) : 0;
        if (is_to_author) {
            toid = "2147483647"
        }
        jump = $('input:radio[name="reply-publish-way"]:checked').val();
        callback = Book.replyCallbackBtm
    } else {
        if (type == "replyBox") {
            callback = Book.replyCallbackBox
        } else {
            if (type.substr(0, type.indexOf("_")) == "floorReplyTxt") {
                toid = type.substr(type.indexOf("_") + 1);
                callback = Book.replyCallbackFloor
            } else {
                if (type.substr(0, type.indexOf("_")) == "floorLayerReplyTxt") {
                    toid = type.split("_")[1];
                    txtid = type.split("_")[2];
                    callback = Book.replyCallbackFloorLayer
                } else {
                    if (type.substr(0, type.indexOf("_")) == "picReplyTxt") {
                        picid = type.substr(type.indexOf("_") + 1);
                        callback = Book.replyCallbackPic
                    } else {
                        if (type.substr(0, type.indexOf("_")) == "picLayerReplyTxt") {
                            toid = type.split("_")[1];
                            picid = type.split("_")[2];
                            callback = Book.replyCallbackPicLayer
                        } else {
                            if (type == "quickReply") {
                                callback = Book.replyCallbackQuick
                            }
                        }
                    }
                }
    ...
    data.atuser = FB.getAtUser();
    $.post("/index.php?c=Ajax_Publish&a=publishReply", data,
        function(json) {
            if (typeof json != "undefined" || !json) {
                if (json.info == "ok") {
                    Book.isCurReply = 0;
                    callback(json, data);
                    ZeroClipboard && ZeroClipboard.Client.prototype.customReposAll(ZeroClipboard.clients)
                } else {
                    if (json.info == "err") {
                        Layer.tips({
                            content: "<p>" + json.msg + "</p>"
...
Book.replyCallbackBtm = function(json, data) {
    ue.execCommand("cleardoc");
    Layer.alert({
        content: json.tips,
        nextLine: Book.replyTips
    });
    Book.replyBtm.before(json.html);
    return
};
Book.replyCallbackBox = function(json, data) {
    $("#replyBoxContent").val("");
    if (json.info == "ok") {
        Layer.alert({
            content: json.tips,
...
Book.replyCallbackFloorCommon = function(json, data) {
    var replyid = data.txtid || data.toid,
        content = $("#floorReplyTxt_" + replyid),
        target = $("#floorReplyBox_" + replyid);
    content.val("");
    Layer.alert({
        content: json.tips,
        nextLine: Book.replyTips
    });
    var myUrl = "http://my.zol.com.cn/" + userid + "/",
        replyUrl = json.toReplyInfo.userInfo.url,
        replyNick = json.toReplyInfo.nickname,
        cname = target.find("li").length ? "": 'class="first"',
        html = "<li " + cname + '><a href="' + myUrl + '" class="person-face">           <img class="border-radius-s3" width="30" height="30" alt="" src="' + json.userInfo.photo + '"></a>           <div><a href="' + myUrl + '" target="_blank">' + json.nickname + '</a> \u56de\u590d <a href="' + replyUrl + '" target="_blank">' + replyNick + "</a> : " + json.content + '</div>           <div class="comment-function"><div class="reply-link"><span><a href="' + json.editReplyUrl + '">\u7f16\u8f91</a></div>\u521a\u624d</div></li>';
    target.find("ul").append(html);
    return
};
Book.replyCallbackFloor = function(json, data) {
    Book.replyCallbackFloorCommon(json, data)
};
Book.replyCallbackFloorLayer = function(json, data) {
    Book.replyCallbackFloorCommon(json, data)
};
Book.replyCallbackPicCommon = function(json, data) {
    var content = $("#picReplyTxt_" + data.picid),
        li = "";
    content.val("");
    var towho = json.toWhoInfo ? ' \u56de\u590d <a href="' + json.toWhoInfo.userInfo.url + '" target="_blank">' + json.toWhoInfo.nickname + "</a> : ": " : ";
    li += '<li><a href="' + json.userInfo.url + '" class="person-face">        <img class="border-radius-s3" width="30" height="30" alt="" src="' + json.userInfo.photo + '"></a>        <div><a href="' + json.userInfo.url + '" target="_blank">' + json.nickname + "</a>" + towho + json.content + '</div>        <div class="comment-function"><div class="reply-link">		<a href="' + json.editReplyUrl + '" >\u7f16\u8f91</a></div>\u521a\u624d</div></li>';
    return li
};
Book.replyCallbackPic = function(json, data) {
    Layer.alert({
        content: json.tips,
        nextLine: Book.replyTips
    });
    var picid = data.picid;
    var li = Book.replyCallbackPicCommon(json, data);
    $("#picReplyCon_" + picid).find("ul").append(li)
};
Book.replyCallbackPicLayer = function(json, data) {
    Layer.alert({
        content: json.tips,
        nextLine: Book.replyTips
    });
...
Book.replyCallbackQuick = function(json, data) {
    $("#quickReplyCon").val("");
    if (json.info == "ok") {
        $("#quickReplyBox").hide();
        Layer.alert({
...
Book.showFloorReply = function(replyid) {
    var replyid = $(this).attr("data-replyid");
    if ($("#floorReplyBox_" + replyid).css("display") != "none") {
        $("#floorReplyBox_" + replyid).hide();
        ZeroClipboard && ZeroClipboard.Client.prototype.customReposAll(ZeroClipboard.clients);
        return
    }
    var floorName = $("#floorReplyBox_" + replyid).find(".replyerNickname").val();
    var floorUrl = $("#floorReplyBox_" + replyid).find(".replyerUrl").val();
    var replyProc = function() {
        var content = $("#floorReplyTxt_" + replyid).val();
        content = FB.formatFaceContent(content);
        Book.reply(content, "floorReplyTxt_" + replyid)
    };
    Book.getFloorReplyInfo(replyid, floorName, floorUrl);
    Z_Effect.bindKeyCommit($("#floorReplyTxt_" + replyid), replyProc);
    $("#floorReplyBtn_" + replyid).unbind().bind("click", replyProc);
    $("#floorReplyBox_" + replyid).show();
    $("#floorReplyTxt_" + replyid).focus()
};
Book.getFloorReplyInfo = function(replyid, floorName, floorUrl) {
    var data = {
        boardid: boardid,
        bookid: bookid,
        replyid: replyid,
    ...
    $.post("/index.php?c=Ajax_Book&a=getFloorReplyInfo", data,
        function(json) {
            if (typeof json != "undefiend") {
                ...
                $.each(json,
                    function(key, val) {
                        var cname = key == 0 ? 'class="first"': "";
                        li += "<li " + cname + '><a href="' + val.userInfo.url + '" class="person-face">                    <img class="border-radius-s3" width="30" height="30" alt="" src="' + val.userInfo.photo + '"></a>                    <div><a href="' + val.userInfo.url + '" target="_blank">' + val.nickname + '</a> \u56de\u590d                     <a href="' + floorUrl + '" target="_blank">' + floorName + "</a> : " + val.content + '</div>                    <div class="comment-function"><div class="reply-link">                    <!--<span class="report-text"><a href="#">\u4e3e\u62a5</a>|</span>-->';
                        if (val.isAdminer || val.isBoarder) {
                            li += '<span><a href="javascript:;" onclick="javascript:Book.delReply(\'' + val.id + "');\">\u5220\u9664</a>|</span>"
                        }
                        li += '<a href="javascript:;" onclick="javascript:Book.floorLayerReply(\'' + val.to_id + "','" + val.id + "')\">\u56de\u590d</a></div>" + val.formatTime + "</div>                </li>"
                    });
                $("#floorReplyBox_" + replyid).find(".comment-list").empty().append(li);
                ZeroClipboard && ZeroClipboard.Client.prototype.customReposAll(ZeroClipboard.clients)
            }
        },
        "json")
};
Book.floorLayerReply = function(to_id, replyid) {
    $("#floorReplyTxt_" + to_id).focus();
    $("#floorReplyBtn_" + to_id).unbind().bind("click",
        function() {
            var content = $("#floorReplyTxt_" + to_id).val();
            content = FB.formatFaceContent(content);
            Book.reply(content, "floorLayerReplyTxt_" + replyid + "_" + to_id)
        })
};
Book.delReplyByMyself = function(id, obj) {
    ...
    $.post("/index.php?c=Ajax_Book&a=setForbidReply", data,
        function(json) {
            if (typeof json != "undefiend") {
                if (json.info == "ok") {
                    o.closest("li").remove();
...
Book.getPicReplyInfo = function(picid) {
    var data = {
        boardid: boardid,
        bookid: bookid,
        picid: picid,
    ...
    $.post("/index.php?c=Ajax_Book&a=getPicReplyInfo", data,
        function(json) {
            if (json.info == "err") {
                Layer.tips({
                    content: "<p>" + json.msg + "</p>"
            ...
            if (typeof json != "undefiend") {
                var num = json.length;
                var li = "";
                $("#picReplyLayer_" + picid).find(".review-icon .layer").text("\u8bc4\u8bba(" + num + ")");
                $.each(json,
                    function(key, val) {
                        var towho = val.toWhoInfo ? ' \u56de\u590d <a href="' + val.toWhoInfo.userInfo.url + '" target="_blank">' + val.toWhoInfo.nickname + "</a> : ": " : ";
                        li += '<li><a href="' + val.userInfo.url + '" class="person-face">                    <img class="border-radius-s3" width="30" height="30" alt="" src="' + val.userInfo.photo + '"></a>                    <div><a href="' + val.userInfo.url + '" target="_blank">' + val.nickname + "</a>" + towho + val.content + '</div>                    <div class="comment-function"><div class="reply-link">                    <!--<span class="report-text"><a href="#">\u4e3e\u62a5</a>|</span>-->';
                        if (userid == val.userInfo.userId) {
                            li += '<span><a href="javascript:;" onclick="javascript:Book.delReplyByMyself(\'' + val.id + "',this);\">\u5220\u9664</a>|</span>"
                        }
                        li += '<a href="javascript:;" onclick="javascript:Book.picLayerReply(\'' + val.picid + "','" + val.id + "')\">\u56de\u590d</a></div>" + val.formatTime + "</div>                </li>"
                    });
                $("#picReplyCon_" + picid).find(".comment-list").empty().append(li);
                $("#picReplyCon_" + picid).show()
            }
        },
        "json")
};
Book.picLayerReply = function(picid, replyid) {
    $("#picReplyTxt_" + picid).focus();
    $("#picReplyBtn_" + picid).unbind().bind("click",
        function() {
            var content = $("#picReplyTxt_" + picid).val();
            content = FB.formatFaceContent(content);
            Book.reply(content, "picLayerReplyTxt_" + replyid + "_" + picid)
        })
};
Book.delReply = function(flooreplyid, src, tagobj) {
    console.log(tagobj);
    var title = "",
        flag = $(this).attr("data-options");
    switch (flag) {
        case "hidden":
            title = "\u9690\u85cf";
            break;
    ...
    var o = Z_Layer.c("operateBookBox", {
            title: title + "\u56de\u590d",
            reason_title: title + "\u539f\u56e0",
            type: 9,
            sure: delReply
        }),
        replyid = flooreplyid ? flooreplyid: parseInt($(this).parent().attr("data-replyid")),
        config = WEB_CONFIG;
    function delReply() {
        data = {
            bbsid: Detail_Config.bbsid,
            ...
            function(json) {
                if (typeof json != "undefined") {
                    if (json.info == "ok") {
                        switch (src) {
                            case "trydetail":
                                tagobj.parents(".com-item").remove();
                                break;
                            default:
                                Layer.tips({
                                    content:
...
Book.collectBook = function() {
    $.get("/index.php?c=Ajax_Book&a=collectBook&t=" + (new Date()).getTime(), {
            bookid: bookid,
            boardid: boardid,
            bbsid: Detail_Config.bbsid
        },
        function(json) {
            if (json.info == "ok") {
                $("#collectBookBtn").text("\u5df2\u6536\u85cf").addClass("btn-blue-disable").unbind();
                typeof taskGuide !== "undefined" && taskGuide.jumpTo(location.href.toString(), 1500)
            } else {
                var disappear = 3000;
                Layer.tips({
...
Book.giveScoreHandle = function(userid, replyid) {
    var oVal,
        box,
        oThis = $(this),
        o = null;
    aVal = $("#addScoreStr").val().split("|");
    box = Layer.scoreBox(aVal);
    o = $("#G_scoreBox");
    o.get(0).box = oThis.closest(".box-shadow");
    $("#scoreBox_userid").val(userid);
    $("#scoreBox_replyid").val(replyid);
    if (o.attr("data-bind") != 1) {
        var oDesc = o.find(".mark-scroll");
        Z_Effect.seledEffect(o.find(".num-list "), {
                c: "select"
            },
            function() {
                $("#scoreBox_selScore").val($(this).attr("data-value"))
            });
        Z_Effect.seledEffect(oDesc, {
            h: "mouseover"
        });
        Z_Effect.seledEffect(oDesc, {
                h: "click"
            ...
            function() {
                $("#scoreBox_content").val($(this).html())
            });
        $("#scoreBox_subBtn").bind("click",
            function() {
                var sCon,
                    oScore = $("#scoreBox_selScore"),
                    oCon = $("#scoreBox_content"),
                    oUser = $("#scoreBox_userid"),
                    oReply = $("#scoreBox_replyid");
                sCon = oCon.val();
                if (!sCon) {
                    Layer.tips({
                        content: "<p>\u8bf7\u8f93\u5165\u5185\u5bb9</p>"
                ...
                Book.scoreBook($("#G_scoreBox").get(0).box, box, oUser.val(), sCon, oScore.val(), oReply.val())
            });
        o.attr("data-bind", 1)
    }
};
Book.scoreBook = function(tableBox, box, userid, content, score, replyid) {
    var str = "",
        isNot = $("#scoreBox_notUser").attr("checked") == "checked" ? 1: 0,
        data = {
            bbsid: Detail_Config.bbsid,
            bookid: bookid,
            boardid: boardid,
    ...
    $.get("/index.php?c=Ajax_Book&a=scoreBook&t=" + (new Date()).getTime(), data,
        function(json) {
            var str = "";
            if (json.info == "ok") {
                var oInfoBox = tableBox.find(".scoreInfoBox"),
                    oScoreBox = oInfoBox.find(".score-list");
                box.hide();
                disappear = 1000;
                $("#showScoreBtn").addClass("btn-pink-disable").text("\u5df2\u8bc4\u5206").unbind();
                $(".host-post").find("a[data-options=score]").text("\u5df2\u8bc4\u5206").attr("data-options", "");
                if (oInfoBox.length < 1) {
                    var oNew = null;
                    str += '<div class="section border-radius-s3 scoreInfoBox">' + '<div class="section-head"><h3 class="score"><em>\u8bc4\u5206\uff1a</em>' + '+Z\u91d1\u8c46 <em class="score_num">' + score + '</em>&nbsp;&nbsp;\u5df2\u6709 <em class="score_people">1</em>\u4eba\u53c2\u4e0e\u8bc4\u5206</h3>' + '<span class="unfold-btn pack-up" data-way="hide">\u6536\u8d77</span>' + '<span data-way="show" style="display:none" class="unfold-btn  pack-off open-up" data-replyid="' + replyid + '">' + '\u5c55\u5f00</span></div><table class="score-list">' + json.str + "</div></div>";
                    oNew = $(str);
                    tableBox.find(".post-main").append(oNew);
                    oNew.find(".unfold-btn").on("click",
                        function() {
                            Book.showScoreBox($(this))
                        })
                } else {
                    var curScore = 0,
                        oPeoNum = oInfoBox.find(".score_people"),
                        oNum = oInfoBox.find(".score_num");
                    str += "<tr>" + json.str + "</tr>";
                    oScoreBox.append(str);
                    oNum.html(parseInt(oNum.html(), 10) + parseInt(score, 10));
                    oPeoNum.html(parseInt(oPeoNum.html(), 10) + 1)
                }
                typeof taskGuide !== "undefined" && taskGuide.jumpTo(location.href.toString(), 1500)
            } else {
                disappear = 3000
...
Book.getScoreList = function(page, replyid, callback) {
    var data = {
        bookid: bookid,
        boardid: boardid,
        page: page,
    ...
    $.post("/index.php?c=Ajax_Book&a=getGiveScoreList&t=" + (new Date()).getTime(), data,
        function(json) {
            if (json.info == "ok") {
                callback && callback(json.msg)
            } else {
                Layer.tips({
                    content: "<p>" + json.msg + "</p>"
                })
...
Book.delScoreInfo = function(o, id, replyid) {
    var data = {
        bookid: bookid,
        boardid: boardid,
        replyid: replyid,
    ...
    $.get("/index.php?c=Ajax_Book&a=delGiveScoreInfo&t=" + (new Date()).getTime(), data,
        function(json) {
            if (json.info == "ok") {
                o.closest("tr").remove()
            } else {
...
Book.changeSign = function(userid) {
    var sCon = $("#signContent").val();
    oData = {
        content: sCon,
        userid: userid
    };
    $.get("/index.php?c=Ajax_User&a=changeSign&t=" + (new Date()).getTime(), oData,
        function(json) {
            Layer.tips({
                content: "<p>" + json.msg + "</p>"
            })
...
Book.reportContent = function(replyid, floor) {
    var o = Layer.reportContent();
    $("#report_subBtn").on("click",
        function() {
            var data = null,
                url = $("#report_link").val(),
                sCon = $("#report_content").val(),
                type = $("#G_reportBox").find("input[name=jubao-type]:checked").val();
            if (!url || !sCon || !type) {
                return false
            }
            data = {
            ...
            $.get("/index.php?c=Ajax_Book&a=reportContent&t=" + (new Date()).getTime(), data,
                function(json) {
                    if (json.info == "ok") {
                        o.hide()
                    }
...
Book.showScoreBox = function(o) {
    var way,
        par;
    if (o.attr("data-way") == "show") {
        way = "block"
    } else {
        way = "none"
    }
    par = o.parent();
    if (par.next("table").length < 1) {
        var replyid = o.attr("data-replyid");
        Book.getScoreList(1, replyid,
            function(str) {
                par.parent().append(str)
            })
    }
    o.css("display", "none");
    o.siblings(".unfold-btn").css("display", "");
    par.siblings(".score-list,.page").css("display", way)
};
Book.favourReply = function() {
    var self = this;
    if (self.flag == 1) {
        return false
    }
    ...
    var isLike,
        obj = $(self),
        curNum = parseInt(obj.find("em").html()),
        newNum = 0,
        replyid = parseInt(obj.attr("data-replyid"), 10),
        login = parseInt(obj.attr("data-login"), 10),
        className = "btn-book-hasfavour",
        a = login === 0 ? "noLoginFavourBook": "favourBook",
        data = null;
    if (!isNaN(replyid)) {
        className = "btn-hasfavour";
        a = login === 0 ? "noLoginFavourReply": "favourReply"
    }
    curNum = isNaN(curNum) ? 0: curNum;
    newNum = curNum;
    ...
    Book.favourChange(newNum, isLike, obj, className); (function(a, replyid, className) {
        self.t = setTimeout(function() {
                data = {
                    "boardid": boardid,
                    "bookid": bookid,
                ...
                $.get("/index.php?c=Ajax_Book&a=" + a + "&t=" + (new Date()).getTime(), data,
                    function(json) {
                        if (typeof json != "undefined") {
                            if (json.info == "ok") {
                                login && typeof taskGuide !== "undefined" && taskGuide.jumpTo(location.href.toString(), 1500)
                            } else {
                                var disappear = 3000;
                                Layer.tips({
                                    content: "<p>" + json.msg + "</p>",
...
Book.favourChange = function(num, isLike, obj, className) {
    if (isLike == 1) {
    ...
    var returnItemsLikes = (num == 0) ? "\u8d5e&nbsp;": "\u8d5e&nbsp;<em>+" + num + "</em>";
    obj.html("<span>" + returnItemsLikes + "</span>");
    obj.get(0).flag = 0
};
Z_Dom.delayCallback = function(options) {
    var sMark = "",
        sQuery = "",
        mQuery = "",
        oData = null,
    ...
    for (var i = 0, size = oData.length; i < size; i++) {
        sMark = ",";
        if (oData[i].ismonth) {
            mQuery += sMark + config.tag + "[" + config.prop + '="' + oData[i].ismonth + '"]';
    ...
    oPar.find(mQuery).each(function() {
        options.callback($(this), "month")
    })
};
Book.getLotterMedal = function() {
    var oUsers = WEB_CONFIG.userArr;
    $.get("/index.php?c=Ajax_User&a=getLotterMedal", {
            "data": oUsers
        },
        function(json) {
            if (json.info != "ok") {
            ...
            Z_Dom.delayCallback({
                ...
                data: json.data,
                tag: "table",
                callback: function(self, param) {
                    if (param == "month") {
                        self.find(".picbox").prepend('<span class="zstg-excellent-moderator-ico">\u4f18\u79c0\u7248\u4e3b\u56fe\u6807</span>')
                    } else {
                        self.find(".picbox").prepend('<span class="zstg-love-you-ico">\u795e\u901a\u54e5\u7231\u4f60\u56fe\u6807</span>')
                    }
                }
            })
        },
...
Book.getFloorReplyNum = function() {
    var rids = WEB_CONFIG.replyIdArr,
        data = {
            "bbsid": Detail_Config.bbsid,
            "boardid": boardid,
            "bookid": bookid,
    ...
    //$.post("/index.php?c=Ajax_Book&a=getFloorReplyNum&t=" + (new Date()).getTime(), data,
    //    function(json) {
    //        if (json.info != "ok") {
    //            return false
    //        }
    //        json.data && $.each(json.data,
    //            function(replyid, num) {
    //                var target = "#showFloorReply_" + replyid;
    //                num && $(target).text("\u56de\u590d(" + num + ")")
    //            })
    //    },
    //    "json")
};
Book.getUserInfo = function() {
    var users = WEB_CONFIG.userArr,
        data = {
            "users": users
        };
    //$.post("/index.php?c=Ajax_Book&a=getUserInfo&t=" + (new Date()).getTime(), data,
    //    function(json) {
    //        if (json.info != "ok") {
    //            return false
    //        }
    ...
    //                var tar = $('table.replyList[data-id="' + userid + '"]');
    //                tar.find("a.post-num").html(info.bookNum + "<br>\u5e16\u5b50");
    //                tar.find("a.jing-num").html(info.goodNum + "<br>\u7cbe\u534e");
    //                tar.find("span.post-num").find("a").text(info.bookNum);
    //                tar.find("span.jing-num").find("a").text(info.goodNum)
    //            })
    //    },
    //    "json")
}; (function() {
...
var Z_PersonInfo = (function($, App) {
    var getActiveUserInfo = function() {
        ...
        createSignLayer = function() {
            ...
            var html = ['<div class="popbox" id="GuserInfoLayer" style="width:554px; display:none;">', '<div class="popbox-inner">', '<div class="popbox-head">', '<span class="popbox-close border-radius-s3" onclick="javascript:$(\'#GuserInfoLayer\').hide();">\u5173\u95ed</span>', '<h3><i class="line"></i>\u6211\u8981\u62a5\u540d</h3>', "</div>", '<div class="popbox-main">', '<div class="personal-infor">', '<form id="signForm">', '<ul class="personal-infor-list">', "<li>", '<label class="title"><em>*</em>\u771f\u5b9e\u59d3\u540d\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signName" name="signName" value="">', "</div>", "</li>", '<li style="z-index:3;">', '<label class="title"><em>*</em>\u624b\u673a\u53f7\u7801\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signTele" name="signTele" value="">', "</div>", "</li>", "<li>", '<label class="title">\u6027\u522b\uff1a</label>', '<div class="personal-infor-cont">', '<label><input type="radio" tabindex="2" name="sex" value="0" checked="checked">\u7537</label>', '<label><input type="radio" tabindex="3" name="sex" value="1">\u5973</label>', "</div>", "</li>", '<li style="z-index:3;">', '<label class="title"><em>*</em>QQ\u53f7\u7801\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signQQ"  name="signQQ" value="">', "</div>", "</li>", '<li style="z-index:2;">', '<label class="title">\u90ae\u5bc4\u5730\u5740\uff1a</label>', 
				'<div class="personal-infor-cont">', 
				//'<div class="selectbox2 border-radius-s3" id="province">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="province" />', '<ul class="border-radius" style="display:none;"><li>--<li></ul>', "</div>", 
				//'<div class="selectbox2 border-radius-s3" id="town">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="town" />', '<ul class="border-radius" style="display:none;"><li>--</li></ul>', "</div>", 
				//'<div class="selectbox2 border-radius-s3" id="city">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="city" />', '<ul class="border-radius" style="display:none;"><li>--</li></ul>', "</div>", 
				"<div>", '<textarea class="add-text-area border-radius-s3" name="address"></textarea>', "</div>", 
				'<p style="color:#aaa;">\u8bf7\u586b\u5199\u6709\u6548\u63a5\u6536\u5730\u5740\uff0c\u4ee5\u4fbf\u63a5\u6536\u5956\u54c1</p>', 
				"</div>", "</li>", "<li>", '<label class="title">\u90ae\u653f\u7f16\u7801\uff1a</label>', 
				//'<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signCode" value="" name="zipCode">', '<span class="wrong-tip" style="display:none;">\u90ae\u7f16\u9519\u8bef\uff0c\u8bf7\u8f93\u5165\u6709\u6548\u90ae\u7f16</span>', "</div>", "</li>", 
				"<li>", '<label class="title">\u5fae\u535a\u5730\u5740\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" name="weibo" value="">', "</div>", "</li>", "</ul>", "</form>", '<div class="popbox-btns"><a class="btn-blue border-radius-s3" href="javascript:;">\u786e\u5b9a</a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn-gray border-radius-s3" href="javascript:;">\u91cd\u7f6e</a></div>', "</div>", "</div>", "</div>", "</div>"];
    ...
    var oHead = document.getElementsByTagName("head")[0],
        iCount = 0,
        aLoadJs = ["/Bbs/jquery.easing.min.js", "/Bbs/jQueryRotate.2.2.js"],
...
$(function() {
    var onInput = 0,
        oScoreList = $(".scoreInfoBox"),
        oModifyLimit = $(".mofidyPermisionErr"),
        odelBookTop = $("#delBookTip"),
        odelReplyTop = $(".delReplyTip"),
        isFavourBook = 0,
        oBackTop = $("#oBackTop"),
        date = new Date(),
    ...
    var TitleScroll = $("#bookTitle").height() + $("#bookTitle").offset().top;
    $(window).scroll(function() {
        if ($(window).scrollTop() >= TitleScroll) {
            $("#bookFixedTitle").show();
            $(".host-click-me").show();
            $(".fixed-host-ico").show()
        } else {
    ...
    var id = url.split("?")[1];
    if (id && id.substr(0, 5) == "reply" && $("#" + id).length > 0) {
        var t = $("#" + id).offset().top;
        $(window).scrollTop(t)
    }
    $(".host-click-me").click(function() {
        $(".host-intro").show();
        $(".host-click-me").hide()
    });
    $(".host-intro-arrow").click(function() {
        $(".host-click-me").show();
        $(".host-intro").hide()
    });
    $("#closeFixedTitle").click(function() {
        $("#bookFixedTitle").hide();
        $(window).unbind()
    });
    $(document).ready(function() {
        page && page == 1 && setTimeout(function() {
                Layer.createPicReplyLayer()
            },
            2000)
    });
    Book.getUserInfo();
    Book.getFloorReplyNum();
    if ($("#bookContent p[data-background]").length > 0) {
        $("#bookContent").attr("style", $("#bookContent p[data-background]").attr("data-background"))
    }
    $("#calendarSignTarget").find("i.sign-get-imazamox-close").click(function() {
        $(this).parent().fadeOut(200);
        var time = (new Date(year + "-" + month + "-" + (day + 1) + " 12:00:00"));
        $.cookie("zol:sign:isShowSignTips", 1, {
            expires: time,
            path: "/",
            domain: "zol.com.cn"
        })
    });
    var signCokey = $.cookie("zol:sign:isShowSignTips");
    if (!signCokey) {
        $("#calendarSignTarget").find("span.sign-get-imazamox-tip").show()
    } else {
        $("#calendarSignTarget").find("span.sign-get-imazamox-tip").hide()
    } (function() {
        var t = {};
        $(".level-box").find(".level").hover(function() {
                $(this).next().show()
            },
            function() {
                var self = $(this);
        ...
        $(".level-box").find(".level-pop").hover(function() {
                clearTimeout(t)
            },
            function() {
                $(this).hide()
    ...
    $("#noLoginFavourBook").on("click",
        function() {
            Book.favourReply.call(this)
        });
    $(".options").on("click", 'a[data-options="noLoginFavour"]',
        function() {
            var param,
                attr = $(this).attr("data-param"),
                param = typeof attr == "string" ? attr.split("|") : [];
            Book.favourReply.apply(this, param)
        });
    if (userid) {
        $(".manage-operation").on("click", "a[data-operate]",
            function() {
                var self = $(this),
                ...
                if (action == "showGoodBox" && WEB_CONFIG.writer == userid) {
                    Z_Layer.c("showTips", {
                        content: "\u4e0d\u80fd\u8bbe\u7f6e\u81ea\u5df1\u7684\u5e16\u5b50\u4e3a\u7cbe\u534e\uff01"
                    });
                    return false
                }
                if (action == "showDelBox" && WEB_CONFIG.booktoptype > 0) {
                    Z_Layer.c("showTips", {
                        content: "\u8bf7\u5148\u53d6\u6d88\u7f6e\u9876\u518d\u5220\u9664~"
                    });
                    return false
        ...
        $("[data-role=calendarSign]").calendarSign({
            ...
            "callback": function(signScore) {
                ...
                Layer.alert({
                    content: "Z\u91d1\u8c46&nbsp;+" + signScore + "&nbsp;&nbsp;",
                    nextLine: Book.replyTips
                })
            }
        });
        $("#toTopByMyself").click(function() {
            Layer.confirm({
                title: "\u672c\u5e16\u7f6e\u9876",
                content: "<p>\u672c\u5e16\u7f6e\u9876\u9700\u8981 <em>" + FB.ToTop + "</em> Z\u91d1\u8c46\uff0c\u60a8\u786e\u8ba4\u8981\u7f6e\u9876\u672c\u5e16\u5417\uff1f</p>",
                sure: function() {
                    Book.toTop()
                }
            })
        });
        $("#toLightByMyself").click(function() {
            Layer.confirm({
                title: "\u672c\u5e16\u52a0\u4eae",
                content: "<p>\u672c\u5e16\u52a0\u4eae\u9700\u8981 <em>" + FB.ToHighlight + "</em> Z\u91d1\u8c46\uff0c\u60a8\u786e\u8ba4\u8981\u52a0\u4eae\u672c\u5e16\u5417\uff1f</p>",
                sure: function() {
                    Book.toLight()
                }
            })
        ...
        $("#inviteFriends").click(function() {
            Book.getFriends();
            $("#G_inviteFriend #G_inviteBtn").unbind().bind("click",
                function() {
                    Book.inviteFriends();
                    $("#G_inviteFriend").hide()
                });
            $("#G_inviteFriend #selectAll").unbind().bind("click",
                function() {
                    if ($(this).find("input").attr("checked") == "checked") {
                        $("#G_inFriendList input").attr("checked", true)
                    } else {
                        $("#G_inFriendList input").attr("checked", false)
                    }
                });
            $("#G_inFriendList input").unbind().bind("click",
                function() {
                    if ($(this).attr("checked") != "checked") {
                        $("#G_inviteFriend #selectAll").find("input").attr("checked", false)
                    }
                })
        });
        $(".wrapper").on("click", ".addFollow",
            function() {
                var touid = $(this).attr("touid");
                User.addFollow(touid, this)
            });
        $("#attenBoardBtn").on("click",
            function() {
                ...
                $.get("/index.php?c=Ajax_User&a=attenBoard&r=" + (new Date()).getTime(), oData,
                    function(json) {
                        if (json.info === "ok") {
                            $(self).replaceWith('<span class="border-radius-s3">\u5df2\u5173\u6ce8</span>')
                        }
                    },
                    "json")
            });
        $("#sendAuthorMessage,.sendReplyerMessage").click(function(event) {
            var data = {
                "userid": userid
            };
            $.get("/index.php?c=Ajax_User&a=GetForeverForbidPublish&r=" + (new Date()).getTime(), data,
                function(json) {
                    if (json.info == "ok") {
                        Layer.tips({
                            content: "<p>\u60a8\u7684ID\u5df2\u88ab\u6c38\u7981\uff0c\u4e0d\u5141\u8bb8\u4efb\u4f55\u64cd\u4f5c~</p>"
                        ...
                        Layer.privateMessage();
                        return true
                    }
                },
                "json");
            var touserid = $(this).attr("touserid");
            $("#G_privateMsg").find(".btn-blue").die().live("click",
                function() {
                    var content = $("#G_privateMsg").find("textarea").val().replace(/\s*/, "");
                    if (!content) {
                        Layer.tips({
                            content: "<p>\u5185\u5bb9\u4e0d\u80fd\u4e3a\u7a7a</p>"
                        });
                    ...
                    User.sendPrivateMessage(touserid, content)
                })
        });
        $(".forbidUser").click(function(event) {
            Layer.forbidUser();
            var touserid = $(this).attr("touserid");
            var o = null;
            var o = $("#G_forbiduser");
            ...
            $(".special").bind("click",
                function() {
                    $("#forbiduser_content").val("");
                    $("#forbiduser_content").focus();
                    return false
                });
            Z_Effect.seledEffect(oDesc, {
                h: "mouseover"
            ...
            Z_Effect.seledEffect(oDesc, {
                ...
                function() {
                    $("#forbiduser_content").val($(this).html())
                });
            $("#G_forbiduser").find(".btn-blue").unbind().bind("click",
                function() {
                    var forbidtime = $("#G_forbiduser").find('input[name="days"]:checked').val();
                    var content = $("#G_forbiduser").find("textarea").val().replace(/\s*/, "");
                    if (!content) {
                        Layer.tips({
                            content: "<p>\u5185\u5bb9\u4e0d\u80fd\u4e3a\u7a7a</p>"
                        });
                    ...
                    User.forbidUser(touserid, forbidtime, content)
                })
        });
        $("#collectBookBtn").bind("click",
            function() {
                Book.collectBook()
            });
        $(".changeSignBtn").bind("click",
            function() {
                var oPar = $(this).parent(),
                    sUserid = oPar.attr("data-userid"),
                    sign = oPar.find(".userSign").text();
                Layer.confirm({
                    title: "\u4fee\u6539\u7b7e\u540d",
                    sure: function() {
                        Book.changeSign(sUserid)
                    },
                    bodyclass: "popbox-signUpdate",
                    content: '<div class="popbox-sixin-title">\u8bf7\u8f93\u5165\u60a8\u8981\u4fee\u6539\u7684\u5185\u5bb9\uff1a</div><input style="font-family: \'Microsoft YaHei\',arial;" class="text border-radius-s3" name="" value="' + sign + '" id="signContent">'
                })
            });
        $(".delSignBtn").on("click",
            function() {
                ...
                Layer.confirm({
                    ...
                    bodyclass: "popbox-delete",
                    content: "<p>\u786e\u8ba4\u8981\u5220\u9664\u8be5\u7b7e\u540d\u5417\uff1f</p>"
                })
            });
        $("#lzDel, #lzHidden").on("click",
            function() {
                if (WEB_CONFIG.booktoptype > 0) {
                    Z_Layer.c("showTips", {
                        content: "\u8bf7\u5148\u53d6\u6d88\u7f6e\u9876\u518d\u5220\u9664~"
                    });
                    return false
                } else {
                    Z_List.showDelBox($(this).text())
                }
            });
        $(".options").on("click", "a[data-options]",
            function() {
                var param,
                    f = $(this).attr("data-options"),
                    attr = $(this).attr("data-param"),
                    a = {
                        "edit": "",
                        "hidden": Book.delReply,
                        "delete": Book.delReply,
                        "report": Book.reportContent,
                        "only": "",
                        "favour": Book.favourReply,
                        "score": Book.giveScoreHandle,
                        "reply": Book.showFloorReply
                    };
                param = typeof attr == "string" ? attr.split("|") : [];
                if (typeof a[f] === "undefined") {
                    return false
                }
                a[f].apply(this, param)
            });
        $(".replyList .post-footer").find(".expression").click(function() {
            var replyid = $(this).attr("data-replyid");
            var selector = "#floorReplyTxt_" + replyid;
            var obj = $(this);
            FB.showEmotion(2, selector,
                function() {
                    $("#zolEmotionLayer").css({
                    ...
                    obj.parent().parent().parent().parent().css({
                        position: "relative",
                        "z-index": "20"
                    });
                    obj.parent().append($("#zolEmotionLayer"))
            ...
            ue.addListener("ready",
                function(editor) {
                    $("#publishReply").click(function() {
                        Book.isCurReply == 1;
                        var content = ue.getContent();
                        Book.reply(content, "bottom", ue.getContentTxt().length)
                    });
        ...
        var procReply = function() {
            var content = $("#replyBoxContent").val();
            content = FB.formatFaceContent(content);
            Book.reply(content, "replyBox")
        };
        $("#replyBoxBtn").click(procReply);
        Z_Effect.bindKeyCommit($("#replyBoxContent"), procReply);
        $("#replyBoxEmotion").click(function() {
            FB.showEmotion(2, "#replyBoxContent")
        });
        $("#popReplyBox, .fix-layout .fastReply").bind("click",
            function() {
                Layer.quickReplyBox();
                $("#quickReplyCon").val("").focus()
            });
        oScoreList.on("click", ".scorePage a",
            function(e) {
                var o = $(this),
                    pageBox = o.parent(),
                    link = this.href,
                    page = link.substr(link.indexOf("###") + 3);
                page = isNaN(page) ? 1: page;
                replyid = pageBox.attr("data-reply");
                Book.getScoreList(page, replyid,
                    function(str) {
                        pageBox.siblings("table").remove();
                        pageBox.before(str);
                        pageBox.remove()
                    });
                return false
            });
        oScoreList.on("click", ".delScoreBtn",
            function() {
                var o = $(this),
                    id = o.attr("data-id");
                replyid = o.attr("data-reply");
                Book.delScoreInfo(o, id, replyid)
            });
        oModifyLimit.on("click",
            function() {
                var type = $(this).attr("type"),
                    str = type == "book" ? "\u5e16\u5b50": "\u56de\u590d";
                App.alert("\u60a8\u7684\u7ecf\u9a8c\u4e0d\u8db3" + WEB_CONFIG.modifyLimitScore + "\uff0c\u4e0d\u5141\u8bb8\u7f16\u8f91" + str + "\uff0c\u8bf7\u52aa\u529b\u63d0\u9ad8\u7ecf\u9a8c")
            });
        if (odelBookTop.length) { (function() {
            var cancelDel = odelBookTop.find(".cancelDel"),
                noPass = odelBookTop.find(".noPass"),
                data = {
                    "bbsid": WEB_CONFIG.bbsid,
                    "boardid": boardid,
                    "bookid": bookid
                };
            cancelDel && cancelDel.click(function() {
                postdata("cancelDel", data)
            });
            noPass && noPass.click(function() {
                postdata("appealNotPass", data)
            });
            var postdata = function(fun, data) {
                $.post("/index.php?c=Ajax_Book&a=" + fun + "&t=" + (new Date()).getTime(), data,
                    function(json) {
                        if (json.info == "err") {
                            Layer.tips({
                                content: "<p>" + json.msg + "~</p>"
        ...
        if (odelReplyTop.length) { (function() {
            var cancelDel = odelReplyTop.find(".cancelDel"),
                data = {
                    "bbsid": WEB_CONFIG.bbsid,
                    "boardid": boardid,
                    "bookid": bookid
            ...
            cancelDel && cancelDel.click(function() {
                postdata("cancelDelReply", data, cancelDel.closest(".delReplyTip"))
            });
            var postdata = function(fun, data, obj) {
                data.replyid = obj.attr("data-role"),
                    $.post("/index.php?c=Ajax_Book&a=" + fun + "&t=" + (new Date()).getTime(), data,
                        function(json) {
                            if (json.info == "err") {
                                Layer.tips({
        ...
        Z_Effect.bookVote("book");
        $("#proScoreModule").length > 0 && $("#proScoreModule").on("click", "[data-typeid]",
            function() {
                var self = $("#proScoreModule").get(0);
                if (self.lock != true) {
                    var oSelf = $(this),
                        url = "/index.php?c=Ajax_ProScore&a=addProScore&t=" + (new Date()).getTime(),
                        oData = null,
                        iId = oSelf.attr("data-typeid");
                    if (iId == "has" || self.has == true) {
                        Layer.tips({
                            content: "<p>\u60a8\u5df2\u7ecf\u8bc4\u8fc7\u5206\u4e86~</p>"
                        });
                        return false
                    ...
                    oData = {
                        ...
                        "boardid": WEB_CONFIG.selfBoardid,
                        "bookid": bookid,
                        "productid": WEB_CONFIG.mainProid,
                        "typeid": iId
                    };
                    $.get(url, oData,
                        function(json) {
                            ...
                            if (json.info == "ok") {
                                var oResult = oSelf.siblings(".examine-result").find(".result-bar"),
                                    oNum = oResult.find(".number"),
                                    iH = parseInt(oResult.css("height"), 10);
                                iH = isNaN(iH) ? 0: iH;
                                oResult.css("height", iH + 1);
                                oNum.html(parseInt(oNum.html(), 10) + 1); ! json.isAdmin && (self.has = true)
                            }
                            self.lock = false
                        },
                        "json")
        ...
        $("#signBtn").bind("click",
            function() {
                if (Detail_Config.showjointag == true) {
                    Active.showSignLayer(1)
                } else {
                    var f = (location.search && location.search.search("f=") > 0) ? location.search.split("f=")[1] : "";
                    Active.showSignLayer(0, "", f)
                }
            });
        $("#nologinSign").bind("click",
            function() {
                Layer.quickLogin()
            });
        $(".pic-listmod-2 li").hover(function() {
            $(this).toggleClass("hover")
        });
        $(".production-listbox").find(".delWork").on("click",
            function() {
                var self = $(this),
                    data = null,
                    bbsid = parseInt(self.attr("data-bbsid")),
                    boardid = parseInt(self.attr("data-boardid")),
                    bookid = parseInt(self.attr("data-bookid"));
                if (isNaN(boardid) || isNaN(bookid)) {
                    return false
                }
                data = {
                    "bbsid": bbsid,
                ...
                $.get("/index.php?c=Ajax_Active&a=delActWork&t=" + (new Date()).getTime(), data,
                    function(json) {
                        App.alert(json.msg);
                        if (json.info === "ok") {
                            self.parent().remove()
    ...
    $("#goToFloor").click(function() {
        var floor = $(this).parent().find("input").val();
        if (!floor || parseInt(floor) <= 0) {
            return false
        }
        $.post("/index.php?c=Ajax_Book&a=goToFloor", {
                boardid: boardid,
                bookid: bookid,
                bbsid: Detail_Config.bbsid,
                floor: floor
    ...
    $(".page-go").find("input").keydown(function(event) {
        if (event.keyCode == 13) {
    ...
    $("#goToPage").click(function() {
        ...
        var data = {
            ...
            bbsid: Detail_Config.bbsid,
            page: page,
            totPage: Detail_Config.totalPage
        };
        $.post("/index.php?c=Ajax_Book&a=goToPage", data,
            function(json) {
                if (typeof json != "undefined") {
                    window.location.href = json.url
                }
    ...
    $("#replyAuthorBtn").click(function() {
        $(window).scrollTop($(".pgs-foot").offset().top);
        ue.focus()
    });
    $("#copyBookUrl").zclip({
        path: "/js/public/ZeroClipboard.swf",
        copy: function() {
            var url = location.origin ? location.origin + location.pathname: "http://" + location.hostname + location.pathname;
            return url.toString()
        },
        afterCopy: function() {
            Layer.tips({
    ...
    $("td.post-title .floor").zclip({
        path: "/js/public/ZeroClipboard.swf",
        copy: function() {
            var floor = parseInt($(this).text()),
                url = location.origin ? location.origin + location.pathname + "#reply" + floor: "http://" + location.hostname + location.pathname + "#reply" + floor;
            return url.toString()
        },
        afterCopy: function() {
            Layer.tips({
                content: "<p>\u5df2\u590d\u5236\u8be5\u697c\u5c42\u94fe\u63a5~</p>"
            })
        }
    });
    oScoreList.find(".unfold-btn").on("click",
        function() {
            Book.showScoreBox($(this))
        });
    $("#showScoreBtn").bind("click",
        function() {
            Book.giveScoreHandle.call(this, $(this).attr("data-userid"))
        });
    $(".infor-list").find(".jd-item").on({
        "mouseenter": function() {
            $(this).find(".jd-layerbox").show()
        },
        "mouseleave": function() {
            $(this).find(".jd-layerbox").hide()
        }
    });
    $(document).keydown(function(event) {
        ...
        if ($("#imgzoom-cover").length && $("#imgzoom-cover").css("display") != "none") {
            switch (event.keyCode) {
                ...
                case 27:
                    $(window).off("resize");
                    $("#imgzoom").parent().css("display", "none");
                    $("#imgzoom-cover").css("display", "none");
                    break;
                default:
                    return;
                    break
        ...
        var next_url = $("#nextPageUrl").val();
        var prev_url = $("#prevPageUrl").val();
        switch (event.keyCode) {
            case 39:
                if (next_url) {
    ...
    $(".adjuncts-pic,.adjuncts-zip").find("a").hover(function() {
            var par = $(this).parent();
            var info = par.attr("data"),
                i = JSON.parse(info),
                target = "#attachLayer_" + i.attachid;
            Layer.showAttaLayer(info, par,
                function() {
                    var t = par.offset().top - $(target).outerHeight(true),
                        l = par.offset().left;
                    $(target).css({
                        "position": "absolute",
                        "top": t,
    ...
    $(".adjuncts-pic a").click(function(event) {
        event.preventDefault();
        var info = $(this).parent().attr("data");
        var href = $(this).attr("href");
        Layer.attachment(info, href)
    });
    if (typeof ue != "undefined") {
        ue.addListener("ready",
            function(editor) {
    ...
    $(".cir_btn").click(function() {
        ...
        if (WEB_CONFIG.bbsid == 6 && WEB_CONFIG.cateid == 5) {
            $.get("/index.php?c=Ajax_Circle&a=isCircleMember&r=" + (new Date()).getTime(), cData,
                function(json) {
                    if (json.info != "ok") {
                        Layer.tips({
                            content: "<p>\u4eb2\uff01\u52a0\u5165\u5708\u5b50\u540e\u624d\u53ef\u4ee5\u53d1\u5e16~</p>"
                        });
                        return false
                    } else {
                        location.href = "/index.php?c=publish&a=default&boardid=" + boardid + "&bbsid=" + WEB_CONFIG.bbsid;
                        return true
                    }
                },
                "json");
    ...
    $("#circleEnterBtn").click(function() {
        ...
        $.getJSON("/index.php?c=Ajax_Circle&a=addCircle", cData,
            function(json) {
                if (json.info == "ok") {
                    $(self).html("\u5df2\u52a0\u5165")
                } else {
    ...
    Book.getLotterMedal();
    Z_Dom.loadFile(WEB_CONFIG.delayLoad);
    $("#callSurvey").survey({
        "pageType": "1"
    });
    Z_Dom.insertShare();
    ...
    Z_Effect.showBox($("#showShareBox"), $("#shareInsertBox"));
    btmLogin.showLayer();
    if (typeof WEB_CONFIG.isPhoneCharge == "object") {
        App.phoneCharge(WEB_CONFIG.isPhoneCharge)
    } (function() {
        var $survey = $("#activeSurvey"),
            surveyId = $survey.find(".surveyId").val();
        $survey.find(".survey-pop-close").click(function() {
            $survey.hide()
        });
        $("#clickToAnswer").css({
            "cursor": "pointer"
            ...
            if (userid) {
                $survey.show();
                ZOL_SURVEY_submit_tip = function() {
                    $survey.hide();
                    var data = {
                        "bbsid": Detail_Config.bbsid,
                        "surveyId": surveyId
                    };
                    $.post("/index.php?c=Ajax_Book&a=getSurveyAnsInfo&t=" + (new Date).getTime(), data,
                        function(json) {
                            if (json.info == "ok") {
                                App.alert("\u7b54\u9898\u5b8c\u6bd5!\u8017\u65f6" + json.time + "\u79d2")
                            } else {
                                if (json.info == "err") {
                                    App.alert(json.msg)
                                }
        ...
        if (!$("#cbsi_survey iframe").length) {
            return false
        }
        var tar = $("#cbsi_survey iframe").get(0),
            win = tar.contentWindow,
            doc = win.document,
            html = doc.documentElement,
            body = doc.body;
        var height = Math.max(body.scrollHeight, body.offsetHeight, html.clientHeight, html.scrollHeight, html.offsetHeight);
        $("#cbsi_survey").css("height", height)
    })(); (function() {
        var target = $(".everyone"),
            span = target.find(".module-header").find("span"),
            tab = target.find(".news-list");
...
var Active = {
    activeId: parseInt($("#activeId").val()) || 0,
    ...
    showSignLayer: function(n, type, taskFlag) {
        var Timer = {},
            n = n || 0,
            type = type || "nor";
        n ? Layer.createSpeSignLayer() : Layer.createSignLayer(type);
        var taskFlag = taskFlag || "";
        Active.getActiveUserInfo();
        ...
        $("#province ul").empty().append($("#defaultProvince li").clone());
        ...
        $("#GsignLayer .popbox-btns .btn-blue").click(function() {
            var tar = $("#GsignLayer"),
                name = tar.find("#signName").val(),
                phone = tar.find("#signTele").val(),
                qq = tar.find("#signQQ").val(),
                isqq = tar.find("#signQQ").parent().prev().find("em"),
                age = tar.find("#signAge"),
                carrer = tar.find("#signCarrer"),
                song = tar.find("#signSong"),
                province = tar.find("#province").find('input[name="province"]').val(),
                isprovince = tar.find("#province").parent().prev().find("em");
            if (!name || !phone) {
                Layer.showTips({
                    type: "warn",
                    content: "\u8bf7\u586b\u5199\u6b63\u786e\u7684\u62a5\u540d\u4fe1\u606f~"
            ...
            if (age.length && carrer.length && song.length) {
                if (!age.val() || !carrer.val() || !song.val()) {
                    Layer.showTips({
                        type: "warn",
                        content: "\u60a8\u7684\u4fe1\u606f\u6ca1\u6709\u586b\u5199\u5b8c\u6574~"
                    });
            ...
            $.post(url, $("#signForm").serialize(),
                function(json) {
                    if (typeof json != "undefined") {
                        if (type == "try") {
                            App.alert(json.msg,
                                function() {
                                    var target = $("#G_TipsLayer"),
                                        inner = target.find(".popbox-inner");
                                    width = inner.width();
                                    height = inner.height();
                                    if (json.info == "err") {
                                        if (json.level && json.level == "entry") {
                                            target.find(".popbox-btns").html('<span class="btn-blue border-radius" onclick="location=' + "'" + "/index.php?c=Try_Detail&a=getOrder&tryid=" + json.data + "&type=0'" + '">\u7acb\u5373\u652f\u4ed8</span>')
                                        }
                                    }
                                    target.css({
                                        "marginTop": parseInt(height / 2, 10) * -1,
                            ...
                            App.alert(json.msg,
                                function() {
                                    if (json.info == "err") {
                                        if (json.level && json.level == "entry") {
                                            $("#G_TipsLayer").find(".popbox-btns").html('<span class="btn-blue border-radius" onclick="location=' + "'" + "http://try.zol.com.cn/index.php?c=Try_Detail&a=getOrder&tryid=" + json.data + "&type=0'" + '">\u7acb\u5373\u652f\u4ed8</span>')
                                        }
                                    }
                                })
                        }
                        if (json.info == "ok") {
                            $("#GsignLayer").hide();
                            if (taskFlag !== "" && typeof taskGuide !== "undefined") {
        ...
        $("#GsignLayer .popbox-btns .btn-gray").click(function() {
            $("#signForm").get(0).reset()
        });
        $("#GsignLayer").show()
    },
    ...
    uploadWork: function() {
        var o = Layer.submitWork(submit);
        function submit() {
            var oData = null,
                actid = Active.activeId,
                oCon = o.find(".content"),
                sCon = oCon.val();
            if (!sCon || isNaN(actid)) {
                return false
            ...
            $.get("/index.php?c=Ajax_Active&a=addActWork&t=" + (new Date()).getTime(), oData,
                function(json) {
                    App.alert(json.msg);
                    if (json.info == "ok") {
                        o.hide();
...
$(function() {
    Active.activeId = WEB_CONFIG.topic_id;
});
Layer.createSpeSignLayer = function() {
    if ($("#GsignLayer").length > 0) {
        App.toCenter("#GsignLayer");
        $("#GsignLayer").show();
        return
    }
    var html = ['<div class="popbox" id="GsignLayer" style="width:554px; display:none;">', '<div class="popbox-inner">', '<div class="popbox-head">', '<span class="popbox-close border-radius-s3" onclick="javascript:$(\'#GsignLayer\').hide();">\u5173\u95ed</span>', '<h3><i class="line"></i>\u6211\u8981\u62a5\u540d</h3>', "</div>", '<div class="popbox-main">', '<div class="personal-infor">', '<form id="signForm">', '<input type="hidden" value="' + Active.activeId + '" name="activeId" />', '<ul class="personal-infor-list">', "<li>", '<label class="title"><em>*</em>\u771f\u5b9e\u59d3\u540d\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signName" name="signName" value="">', "</div>", "</li>", '<li style="z-index:3;">', '<label class="title"><em>*</em>\u624b\u673a\u53f7\u7801\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signTele" name="signTele" value="">', "</div>", "</li>", '<li style="z-index:3;">', '<label class="title">QQ\u53f7\u7801\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signQQ"  name="signQQ" value="">', "</div>", "</li>", "<li>", '<label class="title"><em>*</em>\u6027\u522b\uff1a</label>', '<div class="personal-infor-cont">', '<label><input type="radio" tabindex="2" name="sex" value="0" checked="checked">\u7537</label>', '<label><input type="radio" tabindex="3" name="sex" value="1">\u5973</label>', "</div>", "</li>", "<li>", '<label class="title"><em>*</em>\u5e74\u9f84\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signAge"  name="signAge" value="">', "</div>", "</li>", "<li>", '<label class="title"><em>*</em>\u804c\u4e1a\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signCarrer"  name="signCarrer" value="">', "</div>", "</li>", '<li style="z-index:2;">', '<label class="title"><em>*</em>\u5730\u5740\uff1a</label>', 
		'<div class="personal-infor-cont">', 
		//'<div class="selectbox2 border-radius-s3" id="province">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="province" />', '<ul class="border-radius" style="display:none;"><li>--<li></ul>', "</div> ", 
		//'<div class="selectbox2 border-radius-s3" id="town">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="town" />', '<ul class="border-radius" style="display:none;"><li>--</li></ul>', "</div> ", 
		//'<div class="selectbox2 border-radius-s3" id="city">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="city" />', '<ul class="border-radius" style="display:none;"><li>--</li></ul>', "</div>", 
		"<div>", '<textarea class="add-text-area border-radius-s3" name="address"></textarea>', "</div>", 
		'<p style="color:#aaa;">\u8bf7\u586b\u5199\u6709\u6548\u63a5\u6536\u5730\u5740\uff0c\u4ee5\u4fbf\u63a5\u6536\u5956\u54c1</p>', "</div>", 
		"</li>", '<li style="padding-left: 268px;">', 
		//'<label class="title" style="width: 268px;"><em>*</em>\u6211\u5e38\u7528\u8bfa\u57fa\u4e9a\u624b\u673a\u542c\u7684\u6b4c\u66f2\u540dor\u94c3\u58f0\u540d\uff1a</label>', 
		//'<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signSong"  name="signSong" value="">', "</div>", 
		"</li>", "</ul>", "</form>", '<div class="popbox-btns"><a class="btn-blue border-radius-s3" href="javascript:;">\u786e\u5b9a</a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn-gray border-radius-s3" href="javascript:;">\u91cd\u7f6e</a></div>', "</div>", "</div>", "</div>", "</div>"];
    $("body").append(html.join(""));
    App.toCenter("#GsignLayer")
};
Layer.createSignLayer = function(type) {
    if ($("#GsignLayer").length > 0) {
        App.toCenter("#GsignLayer");
        $("#GsignLayer").show();
        return
    ...
    var type = type;
    if (type == "try") {
        tips = "\u8bf7\u586b\u5199\u6709\u6548\u5730\u5740\uff0c\u4ee5\u4fbf\u6210\u529f\u63a5\u6536\u8bd5\u7528\u4ea7\u54c1"
    } else {
        tips = "\u8bf7\u586b\u5199\u6709\u6548\u63a5\u6536\u5730\u5740\uff0c\u4ee5\u4fbf\u63a5\u6536\u5956\u54c1"
    }
    var html = ['<div class="popbox" id="GsignLayer" style="width:554px; display:none;">', '<div class="popbox-inner">', '<div class="popbox-head">', '<span class="popbox-close border-radius-s3" onclick="javascript:$(\'#GsignLayer\').hide();">\u5173\u95ed</span>', '<h3><i class="line"></i>\u6211\u8981\u62a5\u540d</h3>', "</div>", '<div class="popbox-main">', '<div class="personal-infor">', '<form id="signForm">', '<input type="hidden" value="' + Active.activeId + '" name="activeId" />', '<ul class="personal-infor-list">', "<li>", '<label class="title"><em>*</em>\u771f\u5b9e\u59d3\u540d\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signName" name="signName" value="">', "</div>", "</li>", '<li style="z-index:3;">', '<label class="title"><em>*</em>\u624b\u673a\u53f7\u7801\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signTele" name="signTele" value="">', "</div>", "</li>", "<li>", '<label class="title">\u6027\u522b\uff1a</label>', '<div class="personal-infor-cont">', '<label><input type="radio" tabindex="2" name="sex" value="0" checked="checked">\u7537</label>', '<label><input type="radio" tabindex="3" name="sex" value="1">\u5973</label>', "</div>", "</li>", '<li style="z-index:3;">', '<label class="title"><em>*</em>QQ\u53f7\u7801\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signQQ"  name="signQQ" value="">', "</div>", "</li>", '<li style="z-index:2;">', '<label class="title">\u90ae\u5bc4\u5730\u5740\uff1a</label>', '<div class="personal-infor-cont">', '<div class="selectbox2 border-radius-s3" id="province">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="province" />', '<ul class="border-radius" style="display:none;"><li>--<li></ul>', "</div> ", '<div class="selectbox2 border-radius-s3" id="town">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="town" />', '<ul class="border-radius" style="display:none;"><li>--</li></ul>', "</div> ", '<div class="selectbox2 border-radius-s3" id="city">', '<span class="select-label border-radius-s3"><i class="trangle border-radius-s3"></i>\u8bf7\u9009\u62e9</span>', '<input type="hidden" value="" name="city" />', '<ul class="border-radius" style="display:none;"><li>--</li></ul>', "</div>", "<div>", '<textarea class="add-text-area border-radius-s3" name="address"></textarea>', "</div>", '<p style="color:#aaa;">' + tips + "</p>", "</div>", "</li>", "<li>", '<label class="title">\u90ae\u653f\u7f16\u7801\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" id="signCode" value="" name="zipCode">', '<span class="wrong-tip" style="display:none;">\u90ae\u7f16\u9519\u8bef\uff0c\u8bf7\u8f93\u5165\u6709\u6548\u90ae\u7f16</span>', "</div>", "</li>", "<li>", '<label class="title">\u5fae\u535a\u5730\u5740\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt txt2" name="weibo" value="">', "</div>", "</li>", "</ul>", "</form>", '<div class="popbox-btns"><a class="btn-blue border-radius-s3" href="javascript:;">\u786e\u5b9a</a>&nbsp;&nbsp;&nbsp;&nbsp;<a class="btn-gray border-radius-s3" href="javascript:;">\u91cd\u7f6e</a></div>', "</div>", "</div>", "</div>", "</div>"];
    $("body").append(html.join(""));
    App.toCenter("#GsignLayer")
};
Layer.submitWork = function(callback) {
    var o = $("#GworkSubmit");
    if (o.length > 0) {
        o.find(".content").val("");
        App.toCenter(o);
    ...
    var html = ['<div class="popbox" id="GworkSubmit" style="display:none">', '<div class="popbox-inner"><div class="popbox-head">', '<span class="popbox-close border-radius-s3" onclick="$(\'#GworkSubmit\').hide()">\u5173\u95ed</span>', '<h3><i class="line"></i>\u63d0\u4ea4\u4f5c\u4e1a</h3></div>', '<div class="popbox-main"><div class="personal-infor">', '<ul class="personal-infor-list"><li>', '<label class="title">\u4f5c\u54c1\u8fde\u63a5\uff1a</label>', '<div class="personal-infor-cont">', '<input type="text" class="txt content" style="width:306px;" value="">', '</div></li></ul><div class="popbox-btns">', '<a class="btn-blue border-radius-s3 subBtn" href="###" >\u786e\u5b9a</a>', "&nbsp;&nbsp;&nbsp;&nbsp;", '<a class="btn-gray border-radius-s3" href="###" onclick="$(\'#GworkSubmit\').hide()">\u53d6\u6d88</a></div>', "</div></div></div></div>"];
    o = $(html.join(""));
    callback && o.find(".subBtn").on("click",
        function() {
            callback()
        });
    $("body").append(o);
    ...
    var $ = jQuery,
        h = {
            webUrl: false,
            ipCk: WEB_CONFIG.ipCk,
            stat: function(a, index) {
                var oAction = {
                        "next": "bbs_gallery_next",
                        "init": "bbs_gallery_click"
                    },
                    oScript = null,
                    countUrl = "http://pvtest.zol.com.cn/images/pvevents.gif",
                    picIndex = index + 1;
                ...
                location.hash = "picIndex" + picIndex;
                countUrl += "?t=" + (new Date().getTime()) + "&event=" + oAction[a] + "&ip_ck=" + h.ipCk + "&url=" + webUrl;
                try {
                    oScript = document.getElementById("countHit");
                    document.body.removeChild(oScript)
                } catch(e) {}
                ...
                oScript.id = "countHit";
                document.getElementsByTagName("head")[0].appendChild(oScript);
                _zpv_document_refer = window.location.href.split("#");
                _zpv_document_refer = _zpv_document_refer[0] + "?bbs_gallery=" + picIndex;
                pv_d()
            }
        },
        Lightbox = (function(webSrc) {
            function Lightbox(param) {
                ...
                self.options = $.extend(options, param);
                self.imgs = [];
                self.caches = {};
                self.curImgIndex = 0;
                self.tipsNum = get_cookie("zol_bbs_pic_tips");
                self.oMain = $("#bookContent");
                self.init()
            }
            Lightbox.prototype.init = function() {
                this.bind()
            };
            Lightbox.prototype.addImgs = function(o) {
                var imgSrc = $(o).attr("data-original") ? $(o).attr("data-original") : o.src;
                imgSrc = imgSrc.replace(/t_s\d+?x\d+?\//, "t_s2000x2000/");
                this.imgs.push({
                    link: imgSrc,
                    src: o.src,
                    o: o
            ...
            Lightbox.prototype.bind = function() {
                var sHtml = "",
                    ...
                    index = 1,
                    oMain = self.oMain,
                    oImgs = oMain.find("img[datasrc]");
                if (oImgs.length < 1) {
                    return false
                }
                oImgs.each(function() {
                    var o = $(this);
                    if (o.height() >= 210 && o.width() >= 210) {
                        o.css("cursor", "pointer");
                        o.attr("data-src", index++);
                        self.addImgs(this)
                    }
                });
                if (!self.imgs.length) {
                    $("#showSeePop1, #showSeePop2").hide()
                }
                if ($("#imgzoom").length < 1) {
                    if (typeof(this.tipsNum) != "undefined" && parseInt(this.tipsNum) > 10) {
                        var tipsDisplay = ' style="display:none;"'
                    } else {
                        var tipsDisplay = ""
                    }
                    sHtml = '<div id="imgzoom-cover" style="display: none"></div>' + '<div id="imgzoomOutbox" style="top: 0; display:none;">' + '<div id="lightbox-nav-btnPrev" class="zimg-prev"><i></i></div>' + '<div id="lightbox-nav-btnNext" class="zimg-next"><i></i></div>' + '<div class="zimg-close imgclose"></div>' + '<div class="zimg-close-tip"' + tipsDisplay + '><span class="close" onClick="this.parentNode.style.display=\'none\';return false;"></span></div>' + '<div id="imgzoom" class="" style="display:none;">' + '<div class="zoominner">' + '<img id="imgzoom_img" src="http://icon.zol-img.com.cn/community/public/loading.gif" />' + '<img id="imgzoom_next" style="display:none;" />' + '<div class="zimg-bar clearfix"><a id="imgBlank" class="zimg-adjust-btn" href="###" target="_blank">\u67e5\u770b\u539f\u56fe</a><span class="zimg-number"><em class="imgIndex">0</em>/' + oImgs.length + "</span></div>" + '</div><div class="zimg-adjust"' + tipsDisplay + "></div>" + "</div></div>";
                    $("body").append(sHtml)
                }
                $lightbox = $("#imgzoom");
                this.$overlay = $("#imgzoom-cover");
                this.$prevBtn = $("#lightbox-nav-btnPrev");
                this.$nextBtn = $("#lightbox-nav-btnNext");
                this.$container = $lightbox.find(".zoominner");
                this.$imgBox = $("#imgzoom_img");
                this.$imgNext = $("#imgzoom_next");
                this.$outBox = $("#imgzoomOutbox");
                this.headHeight = 48;
                this.zoominnerTopPadding = 0;
                this.zoominnerRightPadding = this.zoominnerBottomPadding = this.zoominnerLeftPadding = 0;
                self.oMain.on("click", "img[data-src]",
                    function(event) {
                        if (typeof(self.tipsNum) == "undefined") {
                            setCookie("zol_bbs_pic_tips", 1)
                        } else {
                            var tipsNum = parseInt(self.tipsNum);
                            setCookie("zol_bbs_pic_tips", tipsNum + 1)
                        }
                        self.start(event.currentTarget);
                        h.stat("init", self.curImgIndex);
                        return false
                    });
                $("#imgzoomOutbox").on("click",
                    function(event) {
                        if ($(event.target).attr("id") == "imgzoomOutbox") {
                            self.end();
                            return false
                        }
                    });
                if (!- [1, ] && !window.XMLHttpRequest) {
                    $(window).on("scroll",
                        function() {
                            document.getElementById("imgzoomOutbox").style.top = document.documentElement.scrollTop
                        })
                }
                this.$prevBtn.on("click",
                    function() {
                        var index = self.curImgIndex === 0 ? self.imgs.length - 1: self.curImgIndex - 1;
                        if (index == self.curImgIndex) {
                            return false
                        }
                        h.stat("next", index);
                ...
                this.$nextBtn.on("click",
                    function() {
                        var index,
                            curIndex = self.curImgIndex;
                        index = curIndex === self.imgs.length - 1 ? 0: curIndex + 1;
                        if (index == curIndex) {
                            return false
                        }
                        h.stat("next", index);
                ...
                this.$outBox.find(".imgclose").on("click",
                    function() {
                        self.end();
                        return false
                    });
                $("#srcPicSize").on("click",
                    function() {
                        var sizeInfo = self.caches[self.curImgIndex];
                        self.processLayout(sizeInfo.srcH, sizeInfo.srcW, sizeInfo.o.src);
                        sizeInfo = null
                    });
                $("#showSeePop1, #showSeePop2").on("click",
                    function() {
                        if (self.imgs.length > 0) {
                            self.start(self.imgs[0].o)
                        }
                    });
                $(window).bind("resize", $.proxy(this.sizeOverlay, self));
                this.$lightbox = $lightbox;
                $lightbox = lightbox = null
            };
            Lightbox.prototype.start = function(img, way) {
                var self = this,
                    $img = $(img);
                var curIndex = $img.attr("data-src") - 1,
                    srcH = img.height,
                    srcW = img.width;
                self.sizeOverlay();
                self.$overlay.css("display", "block");
                $("#imgzoomOutbox").fadeIn("slow");
                self.changeImage(curIndex);
                way === "anchor" && h.stat("init", curIndex)
            };
            Lightbox.prototype.processSize = function(srcH, srcW) {
                var self = this,
                    iHeight,
                    iWidth,
                    maxImageWidth,
                    ...
                    windowHeight,
                    windowWidth;
                iHeight = newImgHeight = srcH;
                iWidth = newImgWidth = srcW;
                maxImageHeight = self.options.maxImageWidth;
                if ((iWidth > self.options.maxImageWidth) || (iHeight > maxImageHeight)) {
                    windowWidth = $(window).width();
                    windowHeight = $(window).height();
                    maxImageWidth = windowWidth - self.zoominnerLeftPadding - self.zoominnerRightPadding - 20;
                    maxImageHeight = windowHeight - self.zoominnerTopPadding - self.zoominnerBottomPadding - 100;
                    if ((iWidth / maxImageWidth) > (iHeight / maxImageHeight)) {
                        newImgWidth = maxImageWidth;
                        newImgHeight = parseInt(iHeight / (iWidth / newImgWidth), 10)
                    } else {
                        newImgHeight = maxImageHeight;
                        newImgWidth = parseInt(iWidth / (iHeight / newImgHeight), 10)
                    }
                }
                return {
                    h: newImgHeight,
            ...
            Lightbox.prototype.processLayout = function(h, w, src, imageNumber) {
                var o = this.$imgBox;
                o.css("visibility", "hidden").attr("src", src);
                o.get(0).width = w;
                this.$lightbox.fadeIn(500);
                this.sizeContainer(w, h);
                if (typeof imageNumber === "number") {
                    var picInfo = this.caches[imageNumber];
                    o.attr({
                        srcH: picInfo.srcH,
                        srcW: picInfo.srcW
                    })
            ...
            Lightbox.prototype.changeImage = function(imageNumber) {
                var self = this,
                    $image = self.$imgBox,
                    src = self.imgs[imageNumber].link;
                this.$lightbox.find(".imgIndex").text(imageNumber + 1);
                $("#imgBlank").attr("href", src);
                if (typeof self.caches[imageNumber] === "object") {
                    var oInfo = self.caches[imageNumber],
                        sizeInfo = self.processSize(oInfo.srcH, oInfo.srcW);
                    self.processLayout(sizeInfo.h, sizeInfo.w, src)
                } else {
                    self.$imgBox.hide().attr("src", "http://icon.zol-img.com.cn/community/public/loading.gif").css({
                        "position": "absolute",
                        "top": "50%",
                        "left": "50%",
                        "width": "128px",
                    ...
                    self.$lightbox.find(".zimg-bar").hide();
                    self.$lightbox.find(".zimg-adjust").hide();
                    self.$outBox.find(".zimg-close-tip").hide();
                    var sizeInfo = self.processSize(self.imgs[imageNumber].o.height, self.imgs[imageNumber].o.width);
                    self.$imgBox.show();
                    self.sizeLoadContainer(sizeInfo.w, sizeInfo.h);
                    self.$lightbox.fadeIn(500);
                    var preloader = new Image();
                    preloader.onload = function() {
                        var srcH = this.height,
                            srcW = this.width,
                            sizeInfo = self.processSize(srcH, srcW),
                            iHeight = sizeInfo.h,
                            iWidth = sizeInfo.w;
                        sizeInfo.o = this;
                        self.caches[imageNumber] = sizeInfo;
                        self.processLayout(iHeight, iWidth, src, imageNumber);
                        sizeInfo = preloader = null
                    };
                    preloader.src = src;
                    var index,
                        curIndex = imageNumber;
                    index = curIndex === self.imgs.length - 1 ? 0: curIndex + 1;
                    var nextSrc = self.imgs[index].link;
                    this.$imgNext.attr("src", nextSrc)
                }
                this.curImgIndex = imageNumber
            };
            Lightbox.prototype.sizeOverlay = function() {
                ...
                var sizeInfo = this.caches[this.curImgIndex];
                if (typeof sizeInfo === "object") {
                    this.sizeContainer(sizeInfo.w, sizeInfo.h)
                }
                sizeInfo = null
            };
            Lightbox.prototype.sizeContainer = function(imageWidth, imageHeight) {
                var self = this;
                var oldWidth = this.$lightbox.width(),
                    oldHeight = this.$lightbox.height(),
                    newWidth = imageWidth + this.zoominnerRightPadding + this.zoominnerLeftPadding,
                    newHeight = imageHeight + this.zoominnerTopPadding + this.zoominnerBottomPadding + this.headHeight;
                self.showImage();
                self.$lightbox.css({
                    width: newWidth,
                    height: newHeight,
            ...
            Lightbox.prototype.sizeLoadContainer = function(imageWidth, imageHeight) {
                var self = this;
                var oldWidth = this.$lightbox.width(),
                    oldHeight = this.$lightbox.height(),
                    newWidth = imageWidth + this.zoominnerRightPadding + this.zoominnerLeftPadding,
            ...
            Lightbox.prototype.showImage = function() {
                this.$imgBox.removeAttr("style");
                this.$imgBox.css("visibility", "").fadeIn("slow");
                this.$lightbox.find(".zimg-bar").show();
                if (typeof(this.tipsNum) == "undefined" || parseInt(this.tipsNum) <= 10) {
                    this.$lightbox.find(".zimg-adjust").show();
                    this.$outBox.find(".zimg-close-tip").show()
                }
                this.preloadNeighboringImages()
            };
            Lightbox.prototype.preloadNeighboringImages = function() {
                if (this.imgs.length > this.curImgIndex + 1) {
                    var preloadNext = new Image();
                    preloadNext.src = this.imgs[this.curImgIndex + 1].link
                }
                if (this.curImgIndex > 0) {
                    var preloadPrev = new Image();
                    preloadPrev.src = this.imgs[this.curImgIndex - 1].link
                }
            };
            Lightbox.prototype.end = function() {
                $(window).off("resize", this.sizeOverlay);
                this.$lightbox.parent().css("display", "none");
                this.$overlay.css("display", "none")
            };
            return Lightbox
        })();
    $(function() {
        $("#bookContent img.lazy").lazyload({
            effect: "fadeIn",
            threshold: 200
        });
        ...
        if (anchor) {
            var numstr = anchor.match(/picIndex\d+/);
            if (numstr && numstr[0]) {
                var num = parseInt(numstr[0].match(/\d+/));
                num ? lightbox.start($("img[data-role='gallery']").get(num - 1), "anchor") : ""
            }
        }
    })
}).call(this);
$(function() {
    if (WEB_CONFIG.bbsid == 6 && WEB_CONFIG.boardid == 2 && WEB_CONFIG.bookid == 18405) {
        $(".costZdou").click(function() {
            if (!userid) {
                Layer.tips({
                    content: "<p>\u8bf7\u767b\u5f55\u540e\u53c2\u52a0\u5151\u6362</p>"
                });
                return false
            }
            if (!confirm("\u786e\u5b9a\u53c2\u52a0\u5151\u6362?")) {
            ...
            if (id == 5) {
                var couponId = "330";
                var couponToken = "f43a44"
            } else {
                if (id == 10) {
                    var couponId = "331";
                    var couponToken = "7824f2"
                } else {
                    if (id == 50) {
                        var couponId = "332";
                        var couponToken = "5f9214"
                    } else {
                        if (id == 100) {
                            var couponId = "333";
                            var couponToken = "02d39d"
                        }
                    }
                }
            }
            $.getJSON("http://api.zol.com/index.php?c=Tuan_TuanCoupon&a=GetCouponFromPromo&callback=?", {
                    couponId: couponId,
