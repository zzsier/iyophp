    ...
    <div class="infos">
      ...
      <div class="media-heading meta">
        ...
        <span class="operate pull-right">
          <a data-method="post" id="reply-up-vote-{{ $reply->id }}" href="javascript:void(0);" data-url="{{ route('replies.vote', $reply->id) }}" title="{{ trans('message.Vote Up') }}">
          ...
          @if ($currentUser && ($currentUser->can("manage_topics") || $currentUser->id == $reply->user_id) )
            <a id="reply-delete-{{ $reply->id }}" data-method="delete"  href="javascript:void(0);" data-url="{{ route('replies.destroy', [$reply->id]) }}" title="{{ trans('message.Delete') }}">
