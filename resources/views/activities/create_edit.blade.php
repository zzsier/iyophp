...
@section('content')
    ...
    <link href={{{URL::asset('datetimepicker/bootstrap-datetimepicker.css')}}} type="text/css" rel="stylesheet">
	<script src={{{URL::asset('Bbs/Bbs_publish.js')}}} charset="gbk"></script>
	<script src={{{URL::asset('datetimepicker/bootstrap-datetimepicker.js')}}} charset="gbk"></script>
	<script src={{{URL::asset('datetimepicker/bootstrap-datetimepicker.zh-CN.js')}}} charset="gbk"></script>
    ...
    <script>
        var boardid = '0';
		var image_url = String("{{{ isset($activity)?$activity->image:'' }}}");
        ...
        Publish_Config = {
            ...
            productid : '0',
            manuid    : '0',
            subid     : {{{ isset($activity)?$activity->cate_id:'0' }}},
            pageType  : '',
            topic_id : {{{ isset($activity)?$activity->id:'0' }}},
			url : '/activities',
            image : image_url,
            activity : 'active'
        },WEB_CONFIG = {
		...
		<div class="section border-radius">
            ...
            <div class="filter-area clearfix">
                ...
                <div class="selectbox2" id="bookBrand">
					<div class="input-group date form_date" style="width:220px;" data-date="" data-date-format="yyyy-mm-dd" 
					data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
						<input class="form-control" id="dateInput" name="dateInput" style="width:200px" size="10" 
						placeholder="日期" type="text" value="{{{ isset($activity)?date("Y-m-d", strtotime($activity->departure_time)):"" }}}" readonly>
                ...
                <div class="selectbox2" id="bookModel">
					<input type="text" id="addressInput" name="addressInput" placeholder="地址" size="20" style="width:150px;height:35px" 
					class="txt" value="{{{ isset($activity)?$activity->address:"" }}}" />
                </div>

                <div class="tp-text border-radius">
                    <span class="tp-tip">0/40</span>
					<input type="text" placeholder="请输入标题" id="titleInput" name="titleInput" size="20" 
					style="width:280px;height:35px" class="txt" value="{{{ isset($activity)?$activity->title:"" }}}" />
                </div>
...
{{--
			...
			@endif
                <div class="color-set clearfix">
                    ...
                    <select id="is_top">
                        @if( !isset($activity) || $activity->is_top == 0 )
                ...
                <div class="color-set post-set-top clearfix">
					...
					@if( isset($activity) && $activity->is_excellent == 1 )
                ...
                <div class="color-set post-set-top clearfix">
                    <label for="postSetTop">
					@if( isset($activity) && $activity->is_stick == 1 )
